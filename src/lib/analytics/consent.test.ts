...
describe('consent state', () => {
  ...
  it('reads stored consent and defaults unset fields to denied', () => {
    window.localStorage.setItem(
      consentStorageKey(),
      JSON.stringify({ analytics_storage: 'granted', ad_storage: 'denied' }),
    )
...
describe('Do Not Track detection', () => {
  ...
  beforeEach(() => {
    ...
    originalWindowSignal = Object.getOwnPropertyDescriptor(window, 'doNotTrack')
    originalMsSignal = Object.getOwnPropertyDescriptor(navigator, 'msDoNotTrack')
  })
