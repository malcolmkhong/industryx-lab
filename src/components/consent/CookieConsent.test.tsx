...
describe("CookieConsent", () => {
  ...
  it("does not render when consent has already been granted", () => {
    window.localStorage.setItem(
      "industryx:consent",
      JSON.stringify({
        state: { analytics_storage: "granted", ad_storage: "granted" },
  ...
  it("accept button grants consent and hides the banner", () => {
    ...
    const stored = JSON.parse(
      window.localStorage.getItem("industryx:consent") ?? "{}",
    );
    expect(stored.state.analytics_storage).toBe("granted");
    expect(stored.state.ad_storage).toBe("granted");
  ...
  it("decline button keeps analytics denied and hides the banner", () => {
    ...
    expect(stored.state.analytics_storage).toBe("denied");
  });
